...
struct ImVec4
{
    ...
    ImVec4(float _x[4]) { x = _x[0]; y = _x[1]; z = _x[2]; w = _x[3]; }

    float* ToArray() { float f[4]{ x,y,z,w }; return f; }
...
namespace ImGui
{
    ...
    IMGUI_API               void CustomHeaderWithText(const char* text, float width = 200.f);
    ...
    IMGUI_API bool          Checkbox(const char* label, const char* text, bool* v);
    ...
    IMGUI_API bool          SliderScalar(const char* label, const char* text, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format = NULL, ImGuiSliderFlags flags = 0);
...
enum ImGuiKey_
{
    ...
    ImGuiKey_B,                 // for text edit CTRL+B: run
    ImGuiKey_R,                 // for text edit CTRL+B: refresh all
    ImGuiKey_COUNT
};
