...
bool ImGui::Checkbox(const char* label, const char* text, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImColor uncheck_col = ImColor(80, 80, 80);
    ...
    else if (*v)
    {
        ...
        RenderCheckMark(window->DrawList, ImVec2(check_bb.Min.x, check_bb.Min.y - 2) /*+ ImVec2(pad, pad)*/, check_col, square_sz - pad * 2.0f);
    }
    else if (!*v)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        RenderCheckMark(window->DrawList, ImVec2(check_bb.Min.x, check_bb.Min.y - 2) /*+ ImVec2(pad, pad)*/, uncheck_col, square_sz - pad * 2.0f);
    }

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y - 1 /*+ style.FramePadding.y*/);
    if (g.LogEnabled)
        LogRenderedText(&label_pos, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(label_pos, text);
...
bool ImGui::CheckboxFlagsT(const char* label, T* flags, T flags_value)
{
    ...
    if (!all_on && any_on)
    {
        ...
        pressed = Checkbox(label, label, &all_on);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Max.x, bb.Max.y), frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    else
    {
        ...
        if (axis == ImGuiAxis_X)
            *out_grab_bb = ImRect(grab_pos - grab_sz * 0.2f, bb.Min.y + grab_padding, grab_pos + grab_sz * 0.2f, bb.Max.y - grab_padding);
        else
...
bool ImGui::SliderScalar(const char* label, const char* text, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() - 50;

    const ImVec2 label_size = CalcTextSize(text, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(leftPadding, 0), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f) + ImVec2(leftPadding, 0));
    ...
    // Draw frame
    const ImU32 frame_col = ColorConvertFloat4ToU32(ImVec4(1.f, 1.f, 1.f, 1.f));
    RenderNavHighlight(frame_bb, id);
    RenderFrame(ImVec2(frame_bb.Min.x + leftPadding, frame_bb.Min.y + 9), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 9), frame_col, false, g.Style.FrameRounding);

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(ImRect(frame_bb.Min + ImVec2(leftPadding, 0), frame_bb.Max), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ColorConvertFloat4ToU32(ImVec4(1.f, 1.f, 1.f, 1.f)), style.GrabRounding);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    RenderTextClipped(frame_bb.Min - ImVec2(leftPadding, 0), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.f, 0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), text);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    return SliderScalar(label, label, data_type, p_data, p_min, p_max, format, slider_flags);
}
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    else
    {
        ...
        popup_pos = ImVec2(pos.x + 10, pos.y - 3);
...
namespace ImGui {
    void CustomHeaderWithText(const char* text, float width)
    {
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems)
        {
        ...
        auto splitWidth = width - textSize.x;
        float w1 = splitWidth / 2;

        auto x1 = window->DC.CursorPos.x;
        auto x2 = x1 + w1;

        float y1 = window->DC.CursorPos.y + 7;
        float y2 = y1 + 1.0f;

        window->DC.CursorPos.x += w1;
        const ImRect bb(ImVec2(x1, y1), ImVec2(x2, y2));
        ItemSize(ImVec2(0.f, 0.f));
        if (!ItemAdd(bb, NULL))
            return;
        window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Separator), 2.f);
        SameLine();
        Text(text);
        SameLine();
        ...
        const ImRect bb2(ImVec2(x1, y1), ImVec2(x2, y2));
        ItemSize(ImVec2(0.f, 0.f));
        if (!ItemAdd(bb, NULL))
            return;
        window->DrawList->AddLine(bb2.Min, ImVec2(bb2.Max.x, bb2.Min.y), GetColorU32(ImGuiCol_Separator), 2.f);
    }
}
